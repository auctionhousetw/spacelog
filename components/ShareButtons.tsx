...
export default function ShareButtons({ url, title, watchItem }: ShareButtonsProps) {
  const [copied, setCopied] = useState(false);
  const [saved,  setSaved]  = useState(false);

  useEffect(() => {
    if (!watchItem) return;
    setSaved(getWatchlist().some(x => x.id === watchItem.id));
  }, [watchItem]);
  ...
  const toggleWatch = () => {
    ...
    if (saved) {
      list = list.filter(x => x.id !== watchItem.id);
      setSaved(false);
    } else {
      list = [{ ...watchItem, saved_at: new Date().toISOString() }, ...list];
      setSaved(true);
    }
    ...
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 10 }}>
        ...
        <button
          onClick={toggleWatch}
          style={{
            background: saved ? '#fff8f4' : 'none',
            border: `1px solid ${saved ? '#f0c4a0' : '#ececec'}`,
            borderRadius: 2, padding: '.5rem .25rem', fontSize: '.75rem',
            color: saved ? '#c2632a' : '#aaa', cursor: 'pointer',
        ...
        <button style={{
          ...
          padding: '.5rem .25rem', fontSize: '.75rem', color: '#ccc',
          cursor: 'not-allowed', fontFamily: "'Noto Sans TC', sans-serif",
          display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3,
        }}>
      ...
      {saved && (
        <a href="/watchlist" style={{
          display: 'block', textAlign: 'center', fontSize: '.72rem', color: '#c2632a',
          textDecoration: 'none', marginBottom: 10, fontFamily: "'Noto Sans TC', sans-serif",
        }}>
          → 查看我的收藏清單
        </a>
      )}
      ...
      <a
        ...
        style={{ ...baseBtn, background: '#06C755', color: '#fff', border: 'none', marginBottom: 6 }}
      >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        <a href={fbShareUrl} target="_blank" rel="noopener noreferrer"
          style={{ ...baseBtn, background: '#1877F2', color: '#fff', border: 'none' }}>
